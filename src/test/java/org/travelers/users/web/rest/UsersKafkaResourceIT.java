...
class UsersKafkaResourceIT {
    ...
    void setup() {
        ...
        UsersKafkaResource kafkaResource = new UsersKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/users-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/users-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
